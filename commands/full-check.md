...
- 检查源码中的测试辅助代码是否正确使用了 `#[cfg(feature = "test-utils")]` 门控
- 验证生产构建不包含测试代码: `cargo build --release`

**更多关于 test-utils 的信息**,参考本文件末尾的"🧪 test-utils 特性最佳实践"章节。
...
当项目需要测试辅助功能时,应该使用 **条件编译特性** 来隔离测试代码。

### 什么是 test-utils 特性?

`test-utils` 是一个 Cargo feature,用于在源码中提供测试专用的辅助函数和工具,同时确保生产构建不包含这些代码。
...
- [ ] 如果使用了,在所有 cargo 命令中添加 `--features test-utils`
- [ ] 验证测试辅助代码正确使用了 `#[cfg(feature = "test-utils")]`
- [ ] 确认生产构建不包含测试代码: `cargo build --release`
- [ ] 在报告中注明是否使用了 test-utils 特性
...
# 如果找到,会在后续命令中自动添加 --features test-utils
```
